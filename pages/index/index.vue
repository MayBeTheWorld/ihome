  ...
  <view class="systemBox">
    ...
    <view class="header">
      <view class="profileView">
        <image :src='profile' mode="aspectFit" @click="m"></image>
        <!-- "../../static/my/mine.png" -->
      </view>
      <view class="searchBox">
        <input class="search" placeholder="搜索" type="text"/>
        <image class="icon" mode="aspectFit" src="../../static/index/search.png"></image>
      </view>
      <view class="imageView">
        <image class="icon" mode="aspectFit" src="../../static/index/scan.png" @click="openScan"></image>
      </view>
    </view>
    ...
    <scroll-view class="indexView" scroll-x="true">
      ...
      <view class="styleChooseBox">
        <view :style="{ color: styleItem == 0?'#FEB814':'#363636' }" class="buildingStyle"
              @click="chooseStyle(0)">简欧风
        </view>
        <view :style="{ color: styleItem == 1?'#FEB814':'#363636' }" class="buildingStyle"
              @click="chooseStyle(1)">中式风
        </view>
        <view :style="{ color: styleItem == 2?'#FEB814':'#363636' }" class="buildingStyle"
              @click="chooseStyle(2)">美式风
        </view>
        <view :style="{ color: styleItem == 3?'#FEB814':'#363636' }" class="buildingStyle"
              @click="chooseStyle(3)">日式风
        </view>
        <view :style="{ color: styleItem == 4?'#FEB814':'#363636' }" class="buildingStyle"
              @click="chooseStyle(4)">田园风
        </view>
      </view>

      <!-- 瀑布流展示组件 -->
      <swiper :autoplay="false" :circular="true"
              :current="styleItem" class="waterfallsFlowBox" @change="swiperChange">
        <swiper-item>
          <scroll-view class="waterfallsFlowBox" scroll-y="true">
            <view>
              <custom-waterfalls-flow ref="waterfallsFlowRef" :column="column"
                                      :listStyle="listStyle" :value="data.list0" class="swiper-item">
                <!-- #ifdef MP-WEIXIN -->
                <view v-for="(item,index) in data.list0" :key="index" slot="slot{{index}}" class="flowItem">
                  <view class="descBox">
                    ...
                    <view class="userBox">
                      ...
                      <view class="star" @click="saveArticle(item)">
                        <u-icon :color="collectIds.includes(item.id)?'#FEB814':'#363636'"
        ...
        <swiper-item>
          <scroll-view class="waterfallsFlowBox" scroll-y="true">
            <view>
              <custom-waterfalls-flow ref="waterfallsFlowRef" :column="column" :listStyle="listStyle"
                                      :value="data.list0" class="swiper-item">
                <!-- #ifdef MP-WEIXIN -->
                <view v-for="(item,index) in data.list0" :key="index" slot="slot{{index}}" class="flowItem">
                  <view class="descBox">
                    <view class="desc">{{ item.desc }}</view>
    ...
    <!-- 扫码组件 -->
    <!-- #ifdef APP-PLUS -->
    <view :class="{active: isShowScan}" class="scan-box">
      <!-- <uni-nav-bar if="isShowScan" fixed :status-bar="true" title="扫一扫" @clickLeft="closeScan">
      </uni-nav-bar> -->
      <scan ref="scanRef" @goBack="closeScan" @success="getScanCode"/>
    </view>
    <!-- #endif -->
  </view>
</template>
