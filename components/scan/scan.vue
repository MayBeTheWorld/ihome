...
<script>
	...
	export default defineComponent({
		setup(props, ctx) {
			...
			// 这里需要提前定义，不然退出时会报错：ReferenceError: ××× is not defined
			// const currentWebview = ref(null)
			const barcode = ref(null)
			const goBackBtn = ref(null)
			const openGalleryBtn = ref(null)
			...
			function open() {
				...
				// 创建一个相册按钮
				openGalleryBtn.value = new plus.nativeObj.View('openGalleryBtn', {
						bottom: '100px', // 更新位置
						left: '50px', // 更新位置
						height: '35px',
						width: '35px'
				...
				openGalleryBtn.value.show()
				// 拦截按钮区域触摸事件
				openGalleryBtn.value.interceptTouchEvent(true)
				// 添加到当前 Webview
				currentWebview.append(openGalleryBtn.value)
				// 添加事件监听
				openGalleryBtn.value.addEventListener('click', openGallery)
